...
export default function TipPicks() {
  return (
    <div>
             ...
             topPicks.map((item)=>{
                return(
                    <SplideSlide key={item.id}>
                        ...
                        <div className='absolute w-full h-full bg-blackk/50 rounded-3xl text-white'>
                             <p className='px-2'>{item.title}</p>
                             <p className='px-2 pt-4 font-bold text-white'>{item.price}</p>
